...
function replaceText(word, replacement, text) {
	...
	let myRegExp = new RegExp(word, "g");

	if (word) {
		return text.replace(myRegExp, replacement);
	} else {
		return text;
	}
}
...
function checkWord(word, text) {
	...
	let myRegExp = new RegExp(word, "i");
...
function extractTextInParentheses(str) {
	...
	let myRegExp = /\((.*?)\)/g;
	let result = str.matchAll(myRegExp);
	let arr = [];
...
function countEmails(str) {
	...
	let myRegExp = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g;
	let result = str.match(myRegExp);
...
function findWordOccurrences(str, word) {
	...
	// Повертаємо масив
	let myRegExp = new RegExp(word, "gi");
	let matches = [];
	let match;

	while ((match = myRegExp.exec(str))) {
		matches.push(match.index);
		myRegExp.lastIndex = match.index + 1;
	}
	return matches;
}
...
function replaceWordOccurrences(str, word, newWord) {
	...
	let myRegExp = new RegExp(word, "g");
	return str.replace(myRegExp, newWord);
}
...
function checkRegexMethods(regex) {
	...
	if (regex.dotAll, regex.multiline, regex.sticky) {
		methods.push("dotAll", "multiline", "sticky");
	}
